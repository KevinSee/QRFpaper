...
# # to use fish / m2
# fish_hab %<>%
#   mutate(fish_dens = N / FishWettedArea) %>%
...
# # if using fish / m2 as response
# # predict at all CHaMP sites
# pred_hab_sites = hab_impute %>%
#   mutate(chnk_per_m2 = predict(qrf_mod,
#                               newdata = select(., one_of(covars)),
#                               what = pred_quant),
...
# read it in from .csv once, then save as .rds object (smaller in size)
...
# 
# write_rds(champ_frame_df,
#           "../data/derived_data/champ_frame_data.rds")

champ_frame_df = read_rds("../data/derived_data/champ_frame_data.rds")
...
# get some meta data for GAAs
gaa_meta = readxl::read_excel('../../../QRFcapacity/data/raw/master_sample_pts/GAA_Metadata_20150506.xlsx') %>%
              mutate(FieldName = recode(FieldName,
                                        'ChanlType' = 'Channel_Type')) %>%
              bind_rows(tibble(Covariate = "S2_02_11",
                               Name = "NorWeST Avg. Aug. Temp",
                               Units = "C",
                               SpatialScale = "Site-300m"))
...
write_rds(gaa_meta,
          "../data/derived_data/gaa_metadata.rds")
...
spawn_rec_params %<>%
  bind_rows(qrf_params %>%
              select(one_of(names(spawn_rec_params)))) %>%
