...
```{r corr-fig, results='hide', fig.show = "hold", fig.cap = "Correlation plots of metrics, facted by habitat category."}
...
```{r corr-fig2, results='hide', fig.show = "hold", fig.cap = "Correlation plots of metrics, facted by habitat category."}
hab_corr$catg_cor[[1]][!sapply(hab_corr$catg_cor[[1]], is.null)] %>%
  # map(.f = function(x) {
  #   # rownames(x) <- tibble(Metric = rownames(x)) %>%
  #   #   left_join(hab_dict %>%
